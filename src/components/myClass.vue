...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      canWheelScroll() {
        return this.options.enableScroll
      },
      isHorizontal() {
    ...
    methods: {
      ...
      wheel(e) {
        // 拦截滚动
        if (!this.canWheelScroll) return;
        const {direction} = this.options;
        if (direction !== 1) return; // 只拦截向上的
        e.preventDefault();
        const {deltaY} = e;
        this.yPos += deltaY;
        if (this.yPos > 0) this.yPos = 0;
        let h = this.realBoxHeight / 2 * -1
        if (this.yPos < h) this.yPos = 0;
      },
      touchStart(e) {
